...
class ModelApplicationContent extends Model{
    public function addContent($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "content SET glyp_icon = '".$this->db->escape($data['glyp_icon'])."',replace_image = '".(int) $data['replace_image']."', author = '" . (int) $this->user->getId() . "' ,status = '" . (int) $data['status'] . "', date_added = NOW(), date_modified = NOW(), `type` = '" . (int) $data['type'] . "', comment = '" . (isset($data['comment']) ? (int) $data['comment'] : 0) . "', revision = '" . (isset($data['revision']) ? (int) $data['revision'] : 0) . "', link_id ='".(isset($data['link_id']) ? (int) $data['link_id'] : 0)."'");
    ...
    public function editContent($content_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "content SET  glyp_icon = '".$this->db->escape($data['glyp_icon'])."',replace_image = '".(int) $data['replace_image']."',status = '" . (int) $data['status'] . "',date_modified = NOW(), `type` = '" . (int) $data['type'] . "', comment = '" . (isset($data['comment']) ? (int) $data['comment'] : 0) . "', link_id = '" . (isset($data['link_id']) ? (int) $data['link_id'] : 0) . "'  WHERE content_id = '" . (int) $content_id . "'");
