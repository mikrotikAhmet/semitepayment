...
class ModelDesignMenu extends Model {
    ...
    public function editMenu($menu_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "menu SET status = '" . (isset($data['status']) ? (int) $data['status'] : 0) . "', sort_order = '" . (isset($data['sort_order']) ? (int) $data['sort_order'] : 0) . "',position = '".$this->db->escape($data['position'])."', bottom = '".(int) $data['bottom']."',page_link = '".$this->db->escape($data['page_link'])."' WHERE menu_id = '" . (int) $menu_id . "'");
