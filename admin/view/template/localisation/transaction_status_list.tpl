        ...
        <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form" class="form-horizontal" role="form">
            <div class="table-responsive">
                <table class="table table-bordered table-check">
                    ...
                    <tbody>
                        <?php if ($transaction_statuses) { ?>
                        <?php foreach ($transaction_statuses as $transaction_status) { ?>
                        <tr>
                          <td style="text-align: center;"><?php if ($transaction_status['selected']) { ?>
                            <input type="checkbox" name="selected[]" value="<?php echo $transaction_status['transaction_status_id']; ?>" checked="checked" />
                            <?php } else { ?>
                            <input type="checkbox" name="selected[]" value="<?php echo $transaction_status['transaction_status_id']; ?>" />
                            <?php } ?></td>
                          <td class="left"><?php echo $transaction_status['name']; ?></td>
                          <td class="right"><?php foreach ($transaction_status['action'] as $action) { ?>
