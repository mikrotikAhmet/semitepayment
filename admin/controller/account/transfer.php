...
class ControllerAccountTransfer extends Controller {
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_sale_order->editOrder($this->request->get['withdraw_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && ($this->validateDelete())) {
			foreach ($this->request->post['selected'] as $withdraw_id) {
				$this->model_sale_order->deleteOrder($withdraw_id);
				$this->openbay->deleteOrder($withdraw_id);
			}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&withdraw_id=' . $result['withdraw_id'] . $url, 'SSL')
			);

			if (strtotime($result['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				$action[] = array(
					'text' => $this->language->get('text_edit'),
					'href' => $this->url->link('sale/order/update', 'token=' . $this->session->data['token'] . '&withdraw_id=' . $result['withdraw_id'] . $url, 'SSL')
				);
			}
			...
			$this->data['orders'][] = array(
				...
				'selected'      => isset($this->request->post['selected']) && in_array($result['withdraw_id'], $this->request->post['selected']),
				'action'        => $action
			);
		}
		...
		$this->data['column_withdraw_id'] = $this->language->get('column_withdraw_id');
		...
		$this->data['sort_order'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.withdraw_id' . $url, 'SSL');
		...
		$this->data['filter_withdraw_id'] = $filter_withdraw_id;
	...
	public function getForm() {
		...
		if (!isset($this->request->get['withdraw_id'])) {
			...
			$this->data['action'] = $this->url->link('sale/order/update', 'token=' . $this->session->data['token'] . '&withdraw_id=' . $this->request->get['withdraw_id'] . $url, 'SSL');
		}
		...
		if (isset($this->request->get['withdraw_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$transfer_info = $this->model_sale_order->getOrder($this->request->get['withdraw_id']);
		}

		$this->data['token'] = $this->session->data['token'];

		if (isset($this->request->get['withdraw_id'])) {
			$this->data['withdraw_id'] = $this->request->get['withdraw_id'];
		} else {
			$this->data['withdraw_id'] = 0;
		}

		if (isset($this->request->post['store_id'])) {
			$this->data['store_id'] = $this->request->post['store_id'];
		} elseif (!empty($transfer_info)) {
			$this->data['store_id'] = $transfer_info['store_id'];
		} else {
			$this->data['store_id'] = '';
		}
		...
		if (isset($this->request->post['customer'])) {
			...
			$this->data['customer'] = $transfer_info['customer'];
		} else {
			$this->data['customer'] = '';
		}
		...
		if (isset($this->request->post['customer_id'])) {
			...
			$this->data['customer_id'] = $transfer_info['customer_id'];
		} else {
		...
		if (isset($this->request->post['customer_group_id'])) {
			...
			$this->data['customer_group_id'] = $transfer_info['customer_group_id'];
		} else {
		...
		if (isset($this->request->post['firstname'])) {
			...
			$this->data['firstname'] = $transfer_info['firstname'];
		} else {
			$this->data['firstname'] = '';
		}
		...
		if (isset($this->request->post['lastname'])) {
			...
			$this->data['lastname'] = $transfer_info['lastname'];
		} else {
			$this->data['lastname'] = '';
		}
		...
		if (isset($this->request->post['email'])) {
			...
			$this->data['email'] = $transfer_info['email'];
		} else {
			$this->data['email'] = '';
		}
		...
		if (isset($this->request->post['telephone'])) {
			...
			$this->data['telephone'] = $transfer_info['telephone'];
		} else {
			$this->data['telephone'] = '';
		}
		...
		if (isset($this->request->post['fax'])) {
			...
			$this->data['fax'] = $transfer_info['fax'];
		} else {
			$this->data['fax'] = '';
		}
		...
		if (isset($this->request->post['affiliate_id'])) {
			...
			$this->data['affiliate_id'] = $transfer_info['affiliate_id'];
		} else {
		...
		if (isset($this->request->post['affiliate'])) {
			...
			$this->data['affiliate'] = ($transfer_info['affiliate_id'] ? $transfer_info['affiliate_firstname'] . ' ' . $transfer_info['affiliate_lastname'] : '');
		} else {
			$this->data['affiliate'] = '';
		}
		...
		if (isset($this->request->post['order_status_id'])) {
			...
			$this->data['order_status_id'] = $transfer_info['order_status_id'];
		} else {
		...
		if (isset($this->request->post['comment'])) {
			...
			$this->data['comment'] = $transfer_info['comment'];
		} else {
			$this->data['comment'] = '';
		}
		...
		if (isset($this->request->post['customer_id'])) {
			...
			$this->data['addresses'] = $this->model_sale_customer->getAddresses($transfer_info['customer_id']);
		} else {
		...
		if (isset($this->request->post['payment_firstname'])) {
			...
			$this->data['payment_firstname'] = $transfer_info['payment_firstname'];
		} else {
		...
		if (isset($this->request->post['payment_lastname'])) {
			...
			$this->data['payment_lastname'] = $transfer_info['payment_lastname'];
		} else {
		...
		if (isset($this->request->post['payment_company'])) {
			...
			$this->data['payment_company'] = $transfer_info['payment_company'];
		} else {
		...
		if (isset($this->request->post['payment_company_id'])) {
			...
			$this->data['payment_company_id'] = $transfer_info['payment_company_id'];
		} else {
		...
		if (isset($this->request->post['payment_tax_id'])) {
			...
			$this->data['payment_tax_id'] = $transfer_info['payment_tax_id'];
		} else {
		...
		if (isset($this->request->post['payment_address_1'])) {
			...
			$this->data['payment_address_1'] = $transfer_info['payment_address_1'];
		} else {
		...
		if (isset($this->request->post['payment_address_2'])) {
			...
			$this->data['payment_address_2'] = $transfer_info['payment_address_2'];
		} else {
		...
		if (isset($this->request->post['payment_city'])) {
			...
			$this->data['payment_city'] = $transfer_info['payment_city'];
		} else {
		...
		if (isset($this->request->post['payment_postcode'])) {
			...
			$this->data['payment_postcode'] = $transfer_info['payment_postcode'];
		} else {
		...
		if (isset($this->request->post['payment_country_id'])) {
			...
			$this->data['payment_country_id'] = $transfer_info['payment_country_id'];
		} else {
		...
		if (isset($this->request->post['payment_zone_id'])) {
			...
			$this->data['payment_zone_id'] = $transfer_info['payment_zone_id'];
		} else {
		...
		if (isset($this->request->post['payment_method'])) {
			...
			$this->data['payment_method'] = $transfer_info['payment_method'];
		} else {
		...
		if (isset($this->request->post['payment_code'])) {
			...
			$this->data['payment_code'] = $transfer_info['payment_code'];
		} else {
		...
		if (isset($this->request->post['shipping_firstname'])) {
			...
			$this->data['shipping_firstname'] = $transfer_info['shipping_firstname'];
		} else {
		...
		if (isset($this->request->post['shipping_lastname'])) {
			...
			$this->data['shipping_lastname'] = $transfer_info['shipping_lastname'];
		} else {
		...
		if (isset($this->request->post['shipping_company'])) {
			...
			$this->data['shipping_company'] = $transfer_info['shipping_company'];
		} else {
		...
		if (isset($this->request->post['shipping_address_1'])) {
			...
			$this->data['shipping_address_1'] = $transfer_info['shipping_address_1'];
		} else {
		...
		if (isset($this->request->post['shipping_address_2'])) {
			...
			$this->data['shipping_address_2'] = $transfer_info['shipping_address_2'];
		} else {
		...
		if (isset($this->request->post['shipping_city'])) {
			...
			$this->data['shipping_city'] = $transfer_info['shipping_city'];
		} else {
		...
		if (isset($this->request->post['shipping_postcode'])) {
			...
			$this->data['shipping_postcode'] = $transfer_info['shipping_postcode'];
		} else {
		...
		if (isset($this->request->post['shipping_country_id'])) {
			...
			$this->data['shipping_country_id'] = $transfer_info['shipping_country_id'];
		} else {
		...
		if (isset($this->request->post['shipping_zone_id'])) {
			...
			$this->data['shipping_zone_id'] = $transfer_info['shipping_zone_id'];
		} else {
		...
		if (isset($this->request->post['shipping_method'])) {
			...
			$this->data['shipping_method'] = $transfer_info['shipping_method'];
		} else {
		...
		if (isset($this->request->post['shipping_code'])) {
			...
			$this->data['shipping_code'] = $transfer_info['shipping_code'];
		} else {
		...
		if (isset($this->request->post['order_product'])) {
			...
			$order_products = $this->model_sale_order->getOrderProducts($this->request->get['withdraw_id']);
		} else {
		...
		foreach ($order_products as $order_product) {
			if (isset($order_product['order_option'])) {
				...
				$order_option = $this->model_sale_order->getOrderOptions($this->request->get['withdraw_id'], $order_product['order_product_id']);
			} else {
				$order_option = array();
			}
			...
			if (isset($order_product['order_download'])) {
				...
				$order_download = $this->model_sale_order->getOrderDownloads($this->request->get['withdraw_id'], $order_product['order_product_id']);
			} else {
		...
		if (isset($this->request->post['order_voucher'])) {
			...
			$this->data['order_vouchers'] = $this->model_sale_order->getOrderVouchers($this->request->get['withdraw_id']);
		} else {
		...
		if (isset($this->request->post['order_total'])) {
			...
			$this->data['order_totals'] = $this->model_sale_order->getOrderTotals($this->request->get['withdraw_id']);
		} else {
	...
	public function info() {
		...
		if (isset($this->request->get['withdraw_id'])) {
			$withdraw_id = $this->request->get['withdraw_id'];
		} else {
			$withdraw_id = 0;
		}
	...
	public function addCredit() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($transfer_info && $transfer_info['customer_id']) {
				$this->load->model('sale/customer');

				$credit_total = $this->model_sale_customer->getTotalTransactionsByOrderId($this->request->get['withdraw_id']);

				if (!$credit_total) {
					$this->model_sale_customer->addTransaction($transfer_info['customer_id'], $this->language->get('text_withdraw_id') . ' #' . $this->request->get['withdraw_id'], $transfer_info['total'], $this->request->get['withdraw_id']);
	...
	public function removeCredit() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($transfer_info && $transfer_info['customer_id']) {
				...
				$this->model_sale_customer->deleteTransaction($this->request->get['withdraw_id']);
	...
	public function addReward() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($transfer_info && $transfer_info['customer_id']) {
				...
				$reward_total = $this->model_sale_customer->getTotalCustomerRewardsByOrderId($this->request->get['withdraw_id']);

				if (!$reward_total) {
					$this->model_sale_customer->addReward($transfer_info['customer_id'], $this->language->get('text_withdraw_id') . ' #' . $this->request->get['withdraw_id'], $transfer_info['reward'], $this->request->get['withdraw_id']);
	...
	public function removeReward() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($transfer_info && $transfer_info['customer_id']) {
				...
				$this->model_sale_customer->deleteReward($this->request->get['withdraw_id']);
	...
	public function addCommission() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($transfer_info && $transfer_info['affiliate_id']) {
				$this->load->model('sale/affiliate');

				$affiliate_total = $this->model_sale_affiliate->getTotalTransactionsByOrderId($this->request->get['withdraw_id']);

				if (!$affiliate_total) {
					$this->model_sale_affiliate->addTransaction($transfer_info['affiliate_id'], $this->language->get('text_withdraw_id') . ' #' . $this->request->get['withdraw_id'], $transfer_info['commission'], $this->request->get['withdraw_id']);
	...
	public function removeCommission() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($transfer_info && $transfer_info['affiliate_id']) {
				...
				$this->model_sale_affiliate->deleteTransaction($this->request->get['withdraw_id']);
	...
	public function download() {
		...
		$option_info = $this->model_sale_order->getOrderOption($this->request->get['withdraw_id'], $order_option_id);
	...
	public function invoice() {
		...
		$this->data['text_withdraw_id'] = $this->language->get('text_withdraw_id');
		...
		if (isset($this->request->post['selected'])) {
			...
			$orders[] = $this->request->get['withdraw_id'];
		}

		foreach ($orders as $withdraw_id) {
			$transfer_info = $this->model_sale_order->getOrder($withdraw_id);

			if ($transfer_info) {
				$store_info = $this->model_setting_setting->getSetting('config', $transfer_info['store_id']);
				...
				if ($transfer_info['invoice_no']) {
					$invoice_no = $transfer_info['invoice_prefix'] . $transfer_info['invoice_no'];
				} else {
					$invoice_no = '';
				}

				if ($transfer_info['shipping_address_format']) {
					$format = $transfer_info['shipping_address_format'];
				} else {
				...
				$replace = array(
					'firstname' => $transfer_info['shipping_firstname'],
					'lastname'  => $transfer_info['shipping_lastname'],
					'company'   => $transfer_info['shipping_company'],
					'address_1' => $transfer_info['shipping_address_1'],
					'address_2' => $transfer_info['shipping_address_2'],
					'city'      => $transfer_info['shipping_city'],
					'postcode'  => $transfer_info['shipping_postcode'],
					'zone'      => $transfer_info['shipping_zone'],
					'zone_code' => $transfer_info['shipping_zone_code'],
					'country'   => $transfer_info['shipping_country']
				);

				$shipping_address = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));

				if ($transfer_info['payment_address_format']) {
					$format = $transfer_info['payment_address_format'];
				} else {
					$format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
				}
				...
				$replace = array(
					'firstname' => $transfer_info['payment_firstname'],
					'lastname'  => $transfer_info['payment_lastname'],
					'company'   => $transfer_info['payment_company'],
					'address_1' => $transfer_info['payment_address_1'],
					'address_2' => $transfer_info['payment_address_2'],
					'city'      => $transfer_info['payment_city'],
					'postcode'  => $transfer_info['payment_postcode'],
					'zone'      => $transfer_info['payment_zone'],
					'zone_code' => $transfer_info['payment_zone_code'],
					'country'   => $transfer_info['payment_country']
				);
				...
				$products = $this->model_sale_order->getOrderProducts($withdraw_id);

				foreach ($products as $product) {
					$option_data = array();

					$options = $this->model_sale_order->getOrderOptions($withdraw_id, $product['order_product_id']);
					...
					$product_data[] = array(
						...
						'price'    => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $transfer_info['currency_code'], $transfer_info['currency_value']),
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $transfer_info['currency_code'], $transfer_info['currency_value'])
					);
				}

				$voucher_data = array();

				$vouchers = $this->model_sale_order->getOrderVouchers($withdraw_id);

				foreach ($vouchers as $voucher) {
					$voucher_data[] = array(
						'description' => $voucher['description'],
						'amount'      => $this->currency->format($voucher['amount'], $transfer_info['currency_code'], $transfer_info['currency_value'])
					);
				}

				$total_data = $this->model_sale_order->getOrderTotals($withdraw_id);

				$this->data['orders'][] = array(
					'withdraw_id'	         => $withdraw_id,
					'invoice_no'         => $invoice_no,
					'date_added'         => date($this->language->get('date_format_short'), strtotime($transfer_info['date_added'])),
					'store_name'         => $transfer_info['store_name'],
					'store_url'          => rtrim($transfer_info['store_url'], '/'),
					'store_address'      => nl2br($store_address),
					'store_email'        => $store_email,
					'store_telephone'    => $store_telephone,
					'store_fax'          => $store_fax,
					'email'              => $transfer_info['email'],
					'telephone'          => $transfer_info['telephone'],
					'shipping_address'   => $shipping_address,
					'shipping_method'    => $transfer_info['shipping_method'],
					'payment_address'    => $payment_address,
					'payment_company_id' => $transfer_info['payment_company_id'],
					'payment_tax_id'     => $transfer_info['payment_tax_id'],
					'payment_method'     => $transfer_info['payment_method'],
					'product'            => $product_data,
					'voucher'            => $voucher_data,
					'total'              => $total_data,
					'comment'            => nl2br($transfer_info['comment'])
				);
			}
		}
